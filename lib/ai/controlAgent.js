...
const getPrompt = (userInput) =>
  createPrompt({
    ...
    instructions:[
      ...
      `Use today's date(${new Date().toISOString().split('T')[0]}) as reference date if user doesn't provide any date for get_schedule_events function and add_schedule_event function.`
    ],
    userInput: userInput,
  });
