...
const getMainAgentPrompt = (userInput) => createPrompt({
    ...
    instructions: [
        ...
        // SCHEDULE RECOMMENDATIONS (SPECIAL CASE)
        "If the user asks for schedule recommendations (explicitly or implicitly), generate the recommended schedules and include them properly in the 'suggestedSchedules' array.",
        "For each suggested schedule, fill in all fields clearly: title, description, suggestedDate, suggestedStartTime, suggestedEndTime, and suggestedEventCategory.",
        "Do not omit 'suggestedSchedules' if a recommendation was requested. If not applicable, leave it as an empty array.",
        "When you have to suggest events for the user, you must follow this guideline:",
        "- You are the user's personal assistant. Based on the following selected days and current events, recommend new schedules.",
        "- Do NOT ask for additional information — analyze and provide the best suggestions.",
        "- Format your response according to the assistant guidelines.",
