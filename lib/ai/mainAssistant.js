...
3.  **Structure your 'response' text with appropriate natural language formatting:**
    ...
    *   If the 'Data to present' (or a field like 'results' within it) contains an array of multiple items or sub-responses, ensure you address each item sequentially, applying these formatting guidelines (headings, bullets, narrative) for each item as appropriate to fulfill the overall 'Instructions for your presentation'.
...
`;
        ...
        if (!finalInputToModel || finalInputToModel.trim() === "") {
            console.error("Error in sendMessageToMainAssistant: finalInputToModel is empty or undefined.");
            // Option 1: Throw an error to be caught by the existing catch block
            throw new Error("Cannot send an empty message to the AI model.");
        }

        // Ensure finalInputToModel is a string before wrapping (it should be by this point)
        const messageParts = [{ text: String(finalInputToModel) }]; 
