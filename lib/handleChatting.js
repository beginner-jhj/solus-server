...
import { weatherDataFormatter, googleSearchFormatter, scheduleRecommendationsFormatter,getScheduleEventsFormatter,addScheduleEventFormatter, updateUserProfileFormatter, completeOrDeleteScheduleEventFormatter } from "./tools/formatter.js";
...
export async function handleChatting(userRequest, userMetaData){
    try {
        ...
        const response = await controlAgent(userRequest, userMetaData.chatHistory,userMetaData.userProfileInfo,userMetaData.clientDate,userMetaData.clientTime);
        ...
        console.log("user profile info:",userMetaData.userProfileInfo);
    ...
    for (const functionCall of functionCalls) {
        switch (functionCall.name) {
            ...
            case "get_schedule_events":
                ...
                break;
            case "complete_or_delete_schedule_event":
                results.push(await executeCompleteOrDeleteScheduleEvent(functionCall.args, userMetaData));
                break;
...
async function executeCompleteOrDeleteScheduleEvent({ user_query, event_queries }, userMetaData) {
    try {
        let allEvents = [];
        ...
        const formatted = completeOrDeleteScheduleEventFormatter(identifyRes);
        return {
            name: "complete_or_delete_schedule_event",
            summary: identifyRes.response,
            formattedData: formatted
        };
    } catch (err) {
        throw new Error(`Error occurred while executing complete_or_delete_schedule_event: ${err.message}`);
    }
}
