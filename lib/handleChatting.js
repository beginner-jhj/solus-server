...
export async function handleChatting(userRequest,userMetaData){
    try {
        ...
        if(response.functionUsed.length>0){
            const functionResponse = await executeFunction(response.functionUsed,userMetaData);
...
async function executeFunction(functionCalls, userMetaData){
    for(const functionCall of functionCalls){
        ...
        switch(functionCall.name){
            ...
            case "add_schedule_event":
                return await executeAddScheduleEvent({...functionCall.args,userId:userMetaData.userId});
            case "get_schedule_events":
                // The entire functionCall.args should now be { event_queries: [...] }
                return await executeGetScheduleEvents({userId: userMetaData.userId, ...functionCall.args});
