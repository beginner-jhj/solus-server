...
function formatResponseToMainAssistant(responseToMainAsistant){
    ...
    // Format each function response for readability
    const functionsFormatted = responseToMainAsistant.functionResponse.map(fr => {
        const rawDataString = fr.rawData ? JSON.stringify(fr.rawData) : 'None';
        return `Function Name: ${fr.name}\nSummary: ${fr.summary}\nRaw Data: ${rawDataString}`;
    }).join("\n\n");
...
async function executeGoogleSearch({ query }) {
    try {
        ...
        return {
            ...
            summary,
            rawData: { searchResult: googleSearchResponse }
        };
    } catch (err) {
