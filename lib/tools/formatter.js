...
export function googleSearchFormatter(response){
    try {
        const links = Array.isArray(response.relatedLinks)
            ? response.relatedLinks.map((link, i) => `
            ${i + 1}. 
                Title: ${link.title}
                URL: ${link.url}`)
            : "";
        ...
        Response from use_google_search:
            Search Result: ${response.searchResult}
            Related Links:
                ${links.join("\n")}
        `;
    } catch (err) {
...
export function completeOrDeleteScheduleEventFormatter(identifyRes){
    return `
        Response from complete_or_delete_schedule_event:
