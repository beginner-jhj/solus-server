  ...
  return `
    Conversation context:
        ...
        user profile:
            ...
            user residence:${userProfile?.residence}
            user daily routine:${userProfile?.dailyRoutine}
            user personal goals:${userProfile?.personalGoals}
            user detailed likes:${userProfile?.detailedLikes?.map(([key, value]) => `${key}: ${value}`).join(", ")}
    `;
}
