            ...
            event_queries: {
              ...
              items: {
                ...
                properties: {
                  date: {
                    type: Type.STRING,
                    description: "The specific date to query for events, in YYYY-MM-DD format. The AI model must infer this specific date from user queries like 'today', 'tomorrow', 'next Tuesday', or an explicit date."
                  },
                  time_condition: {
                    type: Type.STRING,
                    description: "Optional. The time condition for this date, e.g., 'before', 'after', 'between'. If omitted, all events for the date are fetched."
                    // optional: true // Implied by not being in 'required' array below
                  },
                  time_reference: {
                    type: Type.STRING,
                    description: "Optional. The reference time (HH:MM 24-hour format) for the time_condition on this date. Required if time_condition is 'before' or 'after'."
                    // optional: true
                  },
                  time_reference_end: {
                    type: Type.STRING,
                    description: "Optional. The end reference time (HH:MM 24-hour format) if time_condition is 'between' on this date. Required if time_condition is 'between'."
                    // optional: true
                  }
                },
                required: ["date"] // 'date' is required for each object in the event_queries array
              }
            }
          },
