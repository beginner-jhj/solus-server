...
export async function weatherService(latitude, longitude, days = 1) {
  try {
    // Validate days parameter (1-3)
    let apiDays = Math.max(1, Math.min(parseInt(days, 10) || 1, 3));

    const response = await fetch(
      `http://api.weatherapi.com/v1/forecast.json?key=${WEATHER_API_KEY}&q=${latitude},${longitude}&days=${apiDays}`,
      { method: "GET" }
    );
